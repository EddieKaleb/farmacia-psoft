...
CREATE TABLE `LOTE_HISTORICO` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `LOTE_ID` INT NOT NULL,
  `VENDA_ID` INT NOT NULL,
  `QUANTIDADE_ATUAL` INT NULL,
  `QUANTIDADE_ANTERIOR` INT NULL,
  INDEX `FK_LOTE_HISTORICO_LOTE` (`LOTE_ID` ASC),
  INDEX `FK_VENDA_HISTORICO_LOTE` (`VENDA_ID` ASC),
  PRIMARY KEY (`ID`),
  CONSTRAINT `fk_LOTE_HISTORICO_LOTE1`
    FOREIGN KEY (`LOTE_ID`)
    REFERENCES `LOTE` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_LOTE_HISTORICO_VENDA1`
    FOREIGN KEY (`VENDA_ID`)
    REFERENCES `VENDA` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
